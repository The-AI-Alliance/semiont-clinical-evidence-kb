...
Flags: `--email` / `--password` to seed an admin user, `--observe` to start a Jaeger sidecar, `--config anthropic` for cloud inference (requires `ANTHROPIC_API_KEY`), `--no-cache` to force a fresh image build.
